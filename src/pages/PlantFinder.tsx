...
const PlantFinder = () => {
  ...
  const handleAnswer = (value: string) => {
    ...
    if (isLastQuestion) {
      // Determine the best category based on answers
      const category = determineBestCategory(answers, value);
  ...
  const determineBestCategory = (prevAnswers: Record<string, string>, lastAnswer: string) => {
    // Priority: purpose answer (from last question)
    if (lastAnswer === 'air-purifying' || lastAnswer === 'succulents' || 
        lastAnswer === 'indoor-plants' || lastAnswer === 'snake') {
      return lastAnswer;
    }
    
    // Fallback logic based on other answers
    if (prevAnswers.maintenance === 'low' || prevAnswers.experience === 'beginner') {
      return 'succulents';
    }
    
    if (prevAnswers.light === 'low') {
      return 'snake';
    }
    
    if (prevAnswers.pets === 'yes') {
      return 'pet-friendly';
    }
              ...
              <div className="grid gap-4">
                {currentQuestion.options.map((option, index) => (
                  ...
                  >
                    <Button
                      variant="outline"
                      className="w-full h-auto py-6 px-6 text-left justify-start hover:border-primary hover:bg-primary/5 transition-all group"
